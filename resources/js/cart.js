...
window.cart = {
    ...
    renderCartPage() {
        ...
        const html = this.data.items.map(item => {
            ...
            const detailUrl = `/produk/${itemSlug}`;
            ...
            return `
                <div class="cart-item p-4 sm:p-6 flex flex-col sm:flex-row gap-4 sm:gap-6 items-start border-b border-white/40 last:border-0 bg-white/10 hover:bg-white/20 transition-colors duration-200" data-item-key="${this.itemKey(item)}">
                    <!-- Image -->
                    <a href="${detailUrl}" class="w-full sm:w-24 h-48 sm:h-24 flex-shrink-0 bg-white/30 rounded-xl overflow-hidden shadow-sm relative group block">
                        ...
                        <div class="absolute inset-0 bg-gradient-to-t from-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                            <i class="fa-solid fa-magnifying-glass text-white text-lg drop-shadow"></i>
                        </div>
                    </a>
                    ...
                    <div class="flex-1 w-full">
                        <div class="flex flex-col sm:flex-row justify-between mb-2 gap-2">
                            <div>
                                <h3 class="font-bold text-slate-800 text-lg leading-tight tracking-tight hover:text-emerald-600 transition-colors">
        ...
        container.innerHTML = this.data.items.map(item => {
            ...
            return `
              <div class="flex items-start gap-4 border-b border-slate-100 pb-4 last:border-0 last:pb-0 group" data-item-key="${this.itemKey(item)}">
                <a href="${detailUrl}" class="w-16 h-16 bg-white rounded-lg overflow-hidden shadow-sm border border-slate-100 flex-shrink-0 block">
                  <img src="${item.image || '/img/placeholder.jpg'}" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" loading="lazy">
                </a>
                <div class="flex-1 min-w-0">
                  <div class="flex justify-between items-start mb-1">
                    <h4 class="font-bold text-slate-800 text-sm truncate pr-2 hover:text-emerald-600 transition-colors">
                      <a href="${detailUrl}">${item.name}</a>
                    </h4>
