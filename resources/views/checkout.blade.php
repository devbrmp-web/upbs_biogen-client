    ...
    <div class="grid grid-cols-1 lg:grid-cols-5 gap-8">
        ...
        <div class="lg:col-span-3 space-y-6">
            ...
            <div class="bg-white/80 backdrop-blur-md shadow-lg rounded-xl p-6 border border-white/50 relative overflow-hidden">
                <div class="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-emerald-600 to-emerald-800 opacity-100"></div>
            ...
            <div class="bg-white/80 backdrop-blur-md shadow-lg rounded-xl p-6 border border-white/50 relative overflow-hidden">
                ...
                <div id="delivery-info" class="hidden mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg text-sm text-yellow-800">
                    ...
                    <div class="mt-3">
                        <button type="button" id="btn-call-center-wa" class="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-emerald-700 text-white hover:bg-emerald-800 transition">
            ...
            <div class="bg-white/80 backdrop-blur-md shadow-lg rounded-xl p-6 border border-white/50 relative overflow-hidden">
                        ...
                        <div id="delivery-info" class="hidden mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg text-sm text-yellow-800">
                            ...
                            <div class="mt-3">
                                <button type="button"
                                   ...
                                   class="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-emerald-700 text-white hover:bg-emerald-800 transition">
                                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.008-.57-.008-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/></svg>
                                    Hubungi Call Center
                                </button>
                            </div>
                ...
                <div class="flex justify-end pt-6 border-t mt-6">
                    <button onclick="saveReceiverForm()" class="bg-emerald-700 text-white px-6 py-2 rounded-lg hover:bg-emerald-800 transition font-medium shadow-sm">
                        Simpan Data
                    </button>
                </div>
            </div>
            <div class="bg-white/80 backdrop-blur-md shadow-lg rounded-xl p-6 border border-white/50">
                ...
                <div class="flex flex-col items-center">
                    ...
                    <div class="mt-3 flex gap-2">
                        ...
                        <button type="button" id="btnSaveCheckoutSig" class="text-sm bg-emerald-700 text-white px-3 py-1 rounded">Simpan</button>
                    </div>
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white/80 backdrop-blur-md shadow-lg rounded-xl p-6 border border-white/50 sticky top-24 relative overflow-hidden">
                ...
                <!-- Pay Button -->
                <button id="btn-pay" onclick="processCheckout()" disabled class="w-full bg-emerald-700 text-white py-3.5 rounded-xl font-bold text-lg shadow-lg shadow-emerald-200 hover:bg-emerald-800 hover:shadow-xl transition disabled:opacity-50 disabled:cursor-not-allowed">
  ...
  <div class="relative mx-auto mt-36 mb-12 w-[92%] max-w-xl bg-white rounded-xl shadow-xl p-6">
    ...
    <div class="prose prose-sm max-w-none text-gray-700 space-y-3">
      ...
      <ul class="list-disc list-inside">
        ...
        <li>Refund dikenakan denda sebesar 2.5% dari total biaya yang dibayarkan.</li>
      </ul>
    </div>
...
<script>
    ...
    // WhatsApp Call Center with Cart Items
    document.addEventListener('DOMContentLoaded', () => {
        const btnWa = document.getElementById('btn-call-center-wa');
        if (!btnWa) return;
        btnWa.addEventListener('click', () => {
            ...
            try {
                ...
                if (items.length > 0) {
                    itemsText = 'Item dibeli:\\n' + items.map(i => {
                        const name = i.name || 'Varietas';
    ...
    function contactCallCenter(e) {
        ...
        else greeting = 'malam'; // Fix logic for night

        // Get user data
        const name = document.getElementById('form-name').value || '[Nama Pembeli]';
        const address = document.getElementById('form-address').value;
        const district = document.getElementById('form-district').value;
        const city = document.getElementById('form-city').value;
        const province = document.getElementById('form-province').value;
        
        let fullAddress = '[Alamat Belum Diisi]';
        if (address) {
            fullAddress = `${address}`;
        ...
        const message = `Selamat ${greeting} ka..

saya ${name}

Ingin menanyakan untuk ongkos kirim
${fullAddress}
